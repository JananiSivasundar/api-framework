...
// adding for multiform data
Given(/I set form-urlencoded data (.*) to (.*)$/, function(key,value){
...
Then(/^I fetch access token from the response (.*)$/, function (value) {
   spec.response().should.have.bodyContains(value);
});

Then('I expect the response body should have access token', async function () {
  spec.expect.jsonLike({"access_token": string()})
  spec.stores('Token', 'access_token');
